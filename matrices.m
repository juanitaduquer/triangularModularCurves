...
possibilities := function(order,q,pm)
  ...
  // In this case the matrix is not diagonalizable and has different eigenvalues
    ...
    for factor in quadratic_factors do
      ...
      if order_PXL(M,q,order) eq order then
        Append(~list, M);
      end if;
    end for;
    return <list, #F>;
...
matrices_triple := function(triple,q,pm)
  semisimple:=false;
  i:=1;
  orderTriple:=[1,2,3];
